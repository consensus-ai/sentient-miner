...
// set_host establishes the hostname and port that sentientd is on.
...
// get_header_for_work fetches a block header from sentientd. This block header is
...
int get_header_for_work(uint8_t *target, uint8_t *header) {
	...
	// Get data from sentientd
	curl_easy_reset(curl);
	curl_easy_setopt(curl, CURLOPT_USERAGENT, "Sentient-Agent");
...
// submit_header submits a block header to sentientd.
