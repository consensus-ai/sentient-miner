/*
 * A cross-platform GPU miner built for Sentient
 * using OpenCL for interacting with the graphics card
 * and libcurl for interacting with the Sentient daemon.
 */
...
// Given a number of cycles per iter, grind nonces will poll Sentient for a block
...
double grindNonces(int cycles_per_iter) {
	...
	if (target[0] != 0 || target[1] != 0) {
		...
		printf("Received corrupt target from Sentient\n");
...
int main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc; i++) {
		...
		case 'h':
			printf("\nUsage:\n\n");
			printf("\t C - cycles per iter: Number of kernel executions between Sentient API calls and hash rate updates\n");
	...
	fflush(stdout);
	FILE *fp;
	const char fileName[] = "./sentient-miner.cl";
	size_t source_size;
	char *source_str;
	fp = fopen(fileName, "r");
	if (!fp) {
